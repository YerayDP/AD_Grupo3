...
public interface inscritoRepository extends JpaRepository <Inscrito, Serializable>{
	...
	@Query(value="SELECT * FROM inscrito i, user u, oferta o WHERE u.id = ?1 AND u.id=o.usuario_id AND o.id = i.id_oferta AND i.fecha_inscripcion < CURRENT_DATE()", nativeQuery = true)
